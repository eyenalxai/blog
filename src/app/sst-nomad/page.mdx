...
We're creating a setup with Nomad, Consul, Docker, Traefik, and SST to enable easy remote service deployment and rolling updates

## VPS

We're using a VPS running Ubuntu, and the final setup consumes approximately 1GB of RAM.

Also, we're using Cloudflare for DNS because it integrates well with Traefik. You can use any other DNS provider, but you'll have to adjust the setup accordingly.

Read more [here](https://doc.traefik.io/traefik/https/acme), [here](https://doc.traefik.io/traefik/user-guides/docker-compose/acme-dns), and [here](https://doc.traefik.io/traefik/https/acme/#providers)
...
[Install CNI](https://developer.hashicorp.com/nomad/docs/networking/cni#create-a-cni-bridge-mode-configuration) thing for bridge networking to work

That page is a bit confusing, in case you don't even care what CNI is:
```shell
export ARCH_CNI=$( [ $(uname -m) = aarch64 ] && echo arm64 || echo amd64) CNI_PLUGIN_VERSION=v1.5.1
```
...
Go to `/etc/consul.d`, do `consul.hcl`, paste this:
```hcl
...
Do `systemctl enable --now consul`, it might hang up, just `ctrl + c` it
...
Do `consul kv put traefik/cf_dns_api_token <cf_zone_api_token>`
...
Do `env $(cat .env | xargs) sst deploy`, it will error out, do `env $(cat .env | xargs) sst deploy` again, everything should be fine
...
Notice that database connection is performed over a public IP, but the port is not open on the host, so it's not accessible from outside (somehow idk)

Do `ss -tuln`, observe that the only things exposed are:
- 22 (SSH)
- 80 (HTTP)
- 443 (HTTPS)
- 4646 (Nomad)
